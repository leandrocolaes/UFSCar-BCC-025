...
int main(void) {
    ...
    ret = system("riscv64-elf-as -march=rv32i -mabi=ilp32 -o riscv.o riscv.asm");
    if (ret != 0) {
    ...
    ret = system("riscv64-elf-ld -m elf32lriscv -Ttext=0x0 -o riscv.elf riscv.o");
    if (ret != 0) {
    ...
    ret = system("riscv64-elf-objdump -d riscv.elf > riscv.dump");
    if (ret != 0) {
